...
export class FormVisitaComponent extends FGenerico{
	@Input() pkVisita: any = 0;
	@Input() detalleVisita: any;
	@Input() formVisita: any;
	@Input() disponibilidad: any;
	@Input() fecha: any;
	@Input() listaRazones: any;
	...
	protected changeFkRazon(): void {
		const pkRazon = this.formVisita.value.fkCatRazonVisita;

		if (pkRazon === '0' || pkRazon === 0) {
			this.formVisita.get('otraRazon')?.setValue(null);
			this.formVisita.get('otraRazon')?.enable();
			return;
		}

		this.formVisita.get('otraRazon')?.disable();
		this.formVisita.get('descripcion')?.setValue(this.listaRazones.find((razon: any) => razon.pkCatRazonVisita == pkRazon).descripcionRazon);
	}
